...
public class MainTrain {
	...
	static void checkCorrelationTrain(CorrelatedFeatures c,int f1, int f2, float a, float b){
		if(c.feature1==f1){
			if(c.feature2!=(f2))
	...
	public static void main(String[] args) {
		...
		if(cf.size()!=2)
			...
			for(CorrelatedFeatures c : cf) {
				checkCorrelationTrain(c,1,3,a1,b1); // 20 points
				checkCorrelationTrain(c,2,4,a2,b2); // 20 points
			}
		...
		for(AnomalyReport ar : reports) {
			if(ar.description.equals("1-3") && ar.timeStep == anomaly)
				anomlyDetected=true;
			else
				falseAlarms++;
		};
